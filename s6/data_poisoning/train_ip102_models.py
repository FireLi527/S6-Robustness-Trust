"""Train comparable five-class IP102 models from candidate-only manifests.
...
from IP102's official, disjoint split files.
"""
...
DEFAULT_RESULTS_ROOT = PROJECT_ROOT / "results" / "ip102_poisoning" / "model_training"
DEFAULT_ARTIFACT_ROOT = Path.home() / "s6-training-artifacts"
...
class CandidateManifestDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        for row in rows:
            ...
            image_path = (IP102_ROOT / row["source_relpath"]).resolve()
            if not image_path.is_relative_to(IP102_ROOT.resolve()):
                raise ValueError(f"Image path escapes IP102 root: {row['source_relpath']}")
    ...
    def __getitem__(self, index: int):
        row = self.rows[index]
        image_path = IP102_ROOT / row["source_relpath"]
...
class OfficialSplitDataset(Dataset):
    """Clean evaluation data from IP102's official val/test split."""
    ...
    def __init__(
        ...
        *,
        ...
        split_file = IP102_ROOT / f"{split}.txt"
    ...
    def __getitem__(self, index: int):
        row = self.rows[index]
        image_path = IP102_ROOT / str(row["source_relpath"])
        with Image.open(image_path) as source:
            image = source.convert("RGB")
...
def load_class_names() -> dict[int, str]:
    ...
    for folder_label, line in enumerate(
        (IP102_ROOT / "classes.txt").read_text(encoding="utf-8").splitlines()
    ):
                ...
                torch.save(
                    {
                        ...
                        "validation_split_sha256": file_sha256(IP102_ROOT / "val.txt"),
                        "test_split_sha256": file_sha256(IP102_ROOT / "test.txt"),
                        "pretrained_weights": (
